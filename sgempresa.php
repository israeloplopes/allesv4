            ...
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
				  ...
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_sgempresa.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Empresas</h4>
                    <p class="card-description"><small> Utilize o campo <b>Buscar</b> para filtrar</small>
                    </p>
                    <!--<table class="table table-striped">-->
					<table class="table-responsive-sm" id="hidden-table-info">
                      <thead>
                        <tr>
                          <th> Código </th>
                          <th> Razão </th>						  
                      ...
                      <tbody>
					  <?php if(!empty($rs_sgempresa)):?>
					  <?php foreach($rs_sgempresa as $sgempresa):?>
                        <tr>
                          <td class="py-1">
							<?=$sgempresa->codemp?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$sgempresa->razemp?></td>
                          <td>
                            <!--<div class="progress">
						  ...
						  <td><?=$sgempresa->cidemp?></td>	
						  <td><?=$sgempresa->foneemp?></td>	
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_sgempresa.php?ID=<?=$sgempresa->codemp?>&amp;TP=1">Edita</a></button>
							<?php
								if ($sgempresa->ativo == 'S'):?>
									<form action="pages/forms/act_block_company.php" class="form-horizontal style-form" name="tiradoar" id="tiradoar" method="post">
										<input type="hidden" name="acao" value="bloqueio">
										<input type="hidden" name="codemp" value="<?=$sgempresa->codemp?>">
										<input type="hidden" name="ativo" value="N">
										<button type="submit" class="btn btn-primary btn-rounded btn-fw">Bloqueia</button>
									</form>
							<?php endif;?>	
							<?php
								if ($sgempresa->ativo == 'N'):?>
									<form action="pages/forms/act_block_company.php" class="form-horizontal style-form" name="tiradoar" id="tiradoar" method="post">
