<?php
	...
	$qry_lffrete = 'select * from lffrete where codemp=:empcod';
	$dts_lffrete = $conexao->prepare($qry_lffrete);
	$dts_lffrete->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_lffrete->execute();
	$rs_lffrete = $dts_lffrete->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Frete Avulso</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_lffrete.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_lffrete.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Fretes Avulsos</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_lffrete)):?>
					  <?php foreach($rs_lffrete as $lffrete):?>
                        <tr>
                          <td class="py-1">
							<?=$lffrete->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$lffrete->tipovenda?></td>
                          <td><?=$lffrete->codvenda?></td>
						  <td><?=date('d/m/Y', strtotime($lffrete->dtentrega)); ?></td>
						  <td><?=$lffrete->recebedor?></td>
						  <td><?=$lffrete->entregador?></td>						  
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_lffrete.php?id=<?=$lffrete->id?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_lffrete.php?id=<?=$lffrete->id?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
