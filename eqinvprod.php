<?php
	...
	$qry_eqinvprod = 'select eqi.* , eq.descprod from eqinvprod  as eqi
					  inner join eqproduto as eq on eqi.codprod=eq.codprod
					  where eqi.codemp=:empcod and eq.codprod=eqi.codemp 
					  limit 100';
	$dts_eqinvprod = $conexao->prepare($qry_eqinvprod);
	$dts_eqinvprod->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_eqinvprod->execute();
	$rs_eqinvprod = $dts_eqinvprod->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Inventário de Produtos</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_eqinvprod.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_eqinvprod.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Inventário de Produtos</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_eqinvprod)):?>
					  <?php foreach($rs_eqinvprod as $eqinvprod):?>
                        <tr>
                          <td class="py-1">
							<?=$eqinvprod->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$eqinvprod->codinvprod?></td>
                          <td>
                            <!--<div class="progress">
						  ...
						  <td align="right"><?=$eqinvprod->descprod?></td>
						  <td align="right"><?=$eqinvprod->codlote?></td>
						  <td><?=$eqinvprod->qtdinvp?></td>
						  <td><?=date('d/m/Y', strtotime($eqinvprod->datainvp)); ?></td>
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_eqinvprod.php?ID=<?=$eqinvprod->ID?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_eqinvprod.php?ID=<?=$eqinvprod->ID?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
