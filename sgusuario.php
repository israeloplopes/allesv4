<?php
	...
	if ($_SESSION['nivelusu'] >= 9900) {
		$conexao = conexao::getInstance();
		$qry_sgusuario = 'select * from sgusuario';
		$dts_sgusuario = $conexao->prepare($qry_sgusuario);
		$dts_sgusuario->execute();
		$rs_sgusuario = $dts_sgusuario->fetchAll(PDO::FETCH_OBJ);
	}
	if ($_SESSION['nivelusu'] >= 9499 && $_SESSION['nivelusu'] <9900) { 
		$conexao = conexao::getInstance();
		$qry_sgusuario = 'select * from sgusuario where codemp=:p1';
		$dts_sgusuario = $conexao->prepare($qry_sgusuario);
	    $dts_sgusuario->bindvalue(':p1',$_SESSION['codemp']);		
		$dts_sgusuario->execute();
		$rs_sgusuario = $dts_sgusuario->fetchAll(PDO::FETCH_OBJ);
	}
	if ($_SESSION['nivelusu'] <= 8999) {
		$conexao = conexao::getInstance();
		$qry_sgusuario = 'select * from sgusuario where codemp=:p1 and idusu=:p2';
		$dts_sgusuario = $conexao->prepare($qry_sgusuario);
	    $dts_sgusuario->bindvalue(':p1',$_SESSION['codemp']);
		$dts_sgusuario->bindvalue(':p2',$_SESSION['idusu']);		
		$dts_sgusuario->execute();
		$rs_sgusuario = $dts_sgusuario->fetchAll(PDO::FETCH_OBJ);
	}	
?>	
          ...
          <div class="content-wrapper">
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_sgempresa.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_sgusuario.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
					...
					<table cellspacing="10" cellpadding="3" class="table-responsive-sm" id="hidden-table-info">
                      <thead>
                        <tr>
                          <th> Código </th>
                          <th> Nome Usuário </th>						  
					  ...
					  <?php foreach($rs_sgusuario as $sgusuario):?>
                        <tr>
                          ...
                          <td><?=$sgusuario->nomeusu?></td>
                          <td>
							<?=$sgusuario->ativousu;?>
                          </td>
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="alt_sgusuario.php?ID=<?=$sgusuario->idusu?>&amp;TP=1">Edita</a></button>
						  </td>
						  <?php if($_SESSION['nivelusu'] >= 8499) :?>
						  <td>
							<?php if ($sgusuario->ativousu == 'S'):?>
								<form action="pages/forms/act_block_user.php" class="form-horizontal style-form" name="tiradoar" id="tiradoar" method="post">
									<input type="hidden" name="acao" value="bloqueio">
									<input type="hidden" name="idusu" value="<?=$sgusuario->idusu?>">
									<input type="hidden" name="ativousu" value="N">
									<button type="submit" class="btn btn-primary btn-rounded btn-fw">Bloqueia</button>
								</form>
							<?php endif;?>	
							<?php if ($sgusuario->ativousu == 'N'):?>
								<form action="pages/forms/act_block_user.php" class="form-horizontal style-form" name="tiradoar" id="tiradoar" method="post">
									<input type="hidden" name="acao" value="desbloqueio">
									<input type="hidden" name="idusu" value="<?=$sgusuario->idusu?>">
									<input type="hidden" name="codemp" value="<?=$_SESSION['codemp']?>">
									<input type="hidden" name="ativousu" value="S">
									<button type="submit" class="btn btn-danger btn-rounded btn-fw">Desbloqueia</button>
								</form>
							<?php endif;?>
						  </td>
						  <?php endif;?>
                          <td>
							<button type="button" class="btn btn-secondary btn-rounded btn-fw"><a href="alt_photo_user.php?ID=<?=$sgusuario->idusu?>">Foto</a></button>
						  </td>
						  <td>
							<button type="button" class="btn btn-dark btn-rounded btn-fw"><a href="alt_pass_user.php?ID=<?=$sgusuario->idusu?>&amp;TP=1">Senha</a></button>
						  </td>
                        </tr>
                        <?php endforeach;?>	
                      </tbody>
