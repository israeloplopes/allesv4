<?php
	...
	$qry_vdvendaentrega = 'select * from vdvendaentrega where codemp=:empcod';
	$dts_vdvendaentrega = $conexao->prepare($qry_vdvendaentrega);
	$dts_vdvendaentrega->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_vdvendaentrega->execute();
	$rs_vdvendaentrega = $dts_vdvendaentrega->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Entregas</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_vdvendaentrega.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_vdvendaentrega.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Entregas</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_vdvendaentrega)):?>
					  <?php foreach($rs_vdvendaentrega as $vdvendaentrega):?>
                        <tr>
                          <td class="py-1">
							<?=$vdvendaentrega->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$vdvendaentrega->tipovenda?></td>
                          <td><?=$vdvendaentrega->codvenda?></td>
						  <td><?=date('d/m/Y', strtotime($vdvendaentrega->dtentrega)); ?></td>
						  <td><?=$vdvendaentrega->recebedor?></td>
						  <td><?=$vdvendaentrega->entregador?></td>						  
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_vdvendaentrega.php?id=<?=$vdvendaentrega->id?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_vdvendaentrega.php?id=<?=$vdvendaentrega->id?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
