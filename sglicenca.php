<?php
	...
	$qry_sglicenca = 'select sgl.*, sge.razemp, sgr.nomerev from sglicenca as sgl
					  inner join sgempresa as sge on sgl.codemp=sge.codemp
					  inner join sgrevenda as sgr on sgl.codrev=sgr.codrev';
	$dts_sglicenca = $conexao->prepare($qry_sglicenca);
	$dts_sglicenca->execute();
	$rs_sglicenca = $dts_sglicenca->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Licenças</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_sglicenca.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_sglicenca.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Licenças</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_sglicenca)):?>
					  <?php foreach($rs_sglicenca as $sglicenca):?>
                        <tr>
                          <td class="py-1">
							<?=$sglicenca->codlicenca?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$sglicenca->nomerev?></td>
                          <td>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=$sglicenca->codemp;?>|<?=$sglicenca->razemp?>
                          </td>
						  <td><?=$sglicenca->vlracordado?></td>	
						  <td><?=$sglicenca->registrado?></td>	
						  <td><?=$sglicenca->ativo?></td>
						  <td><?=$sglicenca->revogado?></td>	
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_sglicenca.php?ID=<?=$sglicenca->ID?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_sglicenca.php?ID=<?=$sglicenca->ID?>&amp;TP=2">Exclui</a></button>
							<button type="button" class="btn btn-info btn-rounded btn-fw"><a href="frm_sglicenca.php?ID=<?=$sglicenca->ID?>&amp;TP=1">Sub-Licenças</a></button>							
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
