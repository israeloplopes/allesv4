<?php
	...
	$qry_fnpagar = 'select fn.*, cpf.nomefor from fnpagar as fn
					  inner join cpforneced as cpf on fn.codfor=cpf.codfor
					  where fn.codemp=:empcod and cpf.codemp=fn.codemp 
					  limit 100';
	$dts_fnpagar = $conexao->prepare($qry_fnpagar);
	$dts_fnpagar->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_fnpagar->execute();
	$rs_fnpagar = $dts_fnpagar->fetchAll(PDO::FETCH_OBJ);
          ...
          <div class="content-wrapper">
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_fnpagar.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_fnpagar.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Contas a Pagar</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_fnpagar)):?>
					  <?php foreach($rs_fnpagar as $fnpagar):?>
                        <tr>
                          <td class="py-1">
							<?=$fnpagar->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$fnpagar->codrec?></td>
						  <td><small><?=$fnpagar->nomefor?></small></td>
                          <td><small>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=$fnpagar->codcompra?></small>
                          </td>
						  <td align="right"><small><?=$fnpagar->vlrpag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrdescpag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrmultapag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrjurospag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrdevpag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrparcpag?></small></td>
						  <td align="rigth"><small><?=$fnpagar->vlrpagopag?></small></td>
						  <td><small><?=date('d/m/Y', strtotime($fnpagar->datapag)); ?></small></td>
						  <td><small><?=$fnpagar->statuspag?></small></td>
						  <td><small><?=$fnpagar->docpag?></small></td>
						  
						  
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_fnpagar.php?id=<?=$fnpagar->id?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_fnpagar.php?id=<?=$fnpagar->id?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
