<?php
	...
	$qry_eqtipomov = 'select * from eqtipomov where codemp=:empcod limit 100';
	$dts_eqtipomov = $conexao->prepare($qry_eqtipomov);
	$dts_eqtipomov->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_eqtipomov->execute();
	$rs_eqtipomov = $dts_eqtipomov->fetchAll(PDO::FETCH_OBJ);
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Tipos de Movimentos</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_eqtipomov.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_eqtipomov.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Tipos de Movimentos</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_eqtipomov)):?>
					  <?php foreach($rs_eqtipomov as $eqtipomov):?>
                        <tr>
                          <td class="py-1">
							<?=$eqtipomov->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$eqtipomov->codtipomov?></td>
                          <td>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=$eqtipomov->desctipomov?>
                          </td>
						  <td><?=$eqtipomov->estoqtipomov?></td>
						  <td><?=$eqtipomov->somavdtipomov?></td>					
						  <td><?=$eqtipomov->fiscaltipomov?></td>
						  <td><?=$eqtipomov->estipomov?></td>						  
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_eqtipomov.php?ID=<?=$eqtipomov->ID?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_eqtipomov.php?ID=<?=$eqtipomov->ID?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
