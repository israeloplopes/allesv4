<?php
	...
	$qry_attarefa = 'select tar.*, usu.nomeusu from attarefa as tar
					 inner join sgusuario as usu on tar.idusuoutro=usu.idusu
					 where tar.codemp=:empcod and tar.idusuoutro=:outro order by tar.data asc';
	$dts_attarefa = $conexao->prepare($qry_attarefa);
	$dts_attarefa->bindValue(':empcod',$_SESSION['codemp']);
	$dts_attarefa->bindValue(':outro',$_SESSION['idusu']);
	$dts_attarefa->execute();
	$rs_attarefa = $dts_attarefa->fetchAll(PDO::FETCH_OBJ);
            ...
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
				  ...
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_attarefa.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
                ...
                <div class="card">
                  <div class="card-body">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_attarefa)):?>
					  <?php foreach($rs_attarefa as $attarefa):?>
                        <tr>
                          <td class="py-1">
							<?=$attarefa->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$attarefa->nomeusu?></td>
                          <td>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=implode("/",array_reverse(explode("-",$attarefa->data)));?>
                          </td>
						  <td><?=$attarefa->hora?></td>
						  <td><?=$attarefa->status?></td>
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_attarefa.php?id=<?=$attarefa->id?>&amp;TP=1">Edita</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
						<tr>
							<td colspan='5'><?=$attarefa->tarefa?></td>
						</tr>
                        <?php endforeach;?>	
                      </tbody>
                    </table>
