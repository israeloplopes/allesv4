<?php
	...
	$qry_tai_tickets = 'select tt.*, te.nomeempresa, ts.servico, ti.tipos from tai_tickets as tt
						...
						where tt.tiposolicita=ti.id and tt.tiposervico=ts.id
						order by status';
	$dts_tai_tickets = $conexao->prepare($qry_tai_tickets);
	$dts_tai_tickets->execute();
	$rs_tai_tickets = $dts_tai_tickets->fetchAll(PDO::FETCH_OBJ);
          ...
          <div class="content-wrapper">
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_tai_tickets.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_tai_tickets.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Tickets</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_tai_tickets)):?>
					  <?php foreach($rs_tai_tickets as $tai_tickets):?>
                        <tr>
                          <td class="py-1">
							<?=$tai_tickets->datacadastro?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$tai_tickets->id?></td>
                          <td>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=$tai_tickets->nomeempresa?>
                          </td>
						  <td>
							<?=substr($tai_tickets->nome,0,10)?>
                          </td>							  
                          <td>
							<?=substr($tai_tickets->tipos,0,9)?>
                          </td>			
                          <td>
							<?=$tai_tickets->servico?>
                          </td>	
                          ...
                          <td>
							<span><b><?=substr($tai_tickets->status,0,100)?></b></span>
                          </td>							  
						...
						<tr>
							<td colspan="8">
								<?=substr($tai_tickets->descritivo,0,180)?>
						</td>
						</tr>
						<tr>									
						 <td colspan='8'>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_tai_tickets.php?id=<?=$tai_tickets->id?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_atendeticket.php?id=<?=$tai_tickets->id?>&amp;TP=2">Trata</a></button>
						  </td>
						 </tr>
