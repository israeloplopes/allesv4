<?php
	...
	$qry_fnreceber = 'select fn.*, vdc.nomecli from fnreceber as fn
					  inner join vdcliente as vdc on fn.codcli=vdc.codcli
					  where fn.codemp=:empcod and vdc.codemp=fn.codemp 
					  limit 100';
	$dts_fnreceber = $conexao->prepare($qry_fnreceber);
	$dts_fnreceber->bindvalue(':empcod',$_SESSION['codemp']);
	$dts_fnreceber->execute();
	$rs_fnreceber = $dts_fnreceber->fetchAll(PDO::FETCH_OBJ);
          ...
          <div class="content-wrapper">
            <div class="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <!--<li class="breadcrumb-item"><a href="fr_fnreceber.php" target="_self">Insere</a></li>
                  <!--<li class="breadcrumb-item active" aria-current="page">Basic tables</li>-->
				  <li class="breadcrumb-item active"><button type="button" class="btn btn-primary btn-rounded btn-fw"><a href="frm_fnreceber.php?ID=0&amp;TP=0">Insere</a></button><li>
                </ol>
              </nav>
            </div>
            <div class="row">
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Consulta Contas a Receber</h4>
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table-responsive-sm" id="hidden-table-info">
                      ...
                      <tbody>
					  <?php if(!empty($rs_fnreceber)):?>
					  <?php foreach($rs_fnreceber as $fnreceber):?>
                        <tr>
                          <td class="py-1">
							<?=$fnreceber->id?>
                            <!--<img src="images/faces-clipart/pic-1.png" alt="image" />-->
                          </td>
                          <td><?=$fnreceber->codrec?></td>
						  <td><small><?=$fnreceber->nomecli?></small></td>
                          <td><small>
                            <!--<div class="progress">
                              <div class="progress-bar bg-success" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>-->
							<?=$fnreceber->codvenda?>/<?=$fnreceber->tipovenda?></small>
                          </td>
						  <td align="right"><small><?=$fnreceber->vlrrec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrdescrec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrmultarec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrjurosrec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrdevrec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrparcrec?></small></td>
						  <td align="rigth"><small><?=$fnreceber->vlrpagorec?></small></td>
						  <td><small><?=date('d/m/Y', strtotime($fnreceber->datarec)); ?></small></td>
						  <td><small><?=$fnreceber->statusrec?></small></td>
						  <td><small><?=$fnreceber->docrec?></small></td>
						  
						  
                          <td>
							<button type="button" class="btn btn-warning btn-rounded btn-fw"><a href="frm_fnreceber.php?id=<?=$fnreceber->id?>&amp;TP=1">Edita</a></button>
							<button type="button" class="btn btn-danger btn-rounded btn-fw"><a href="frm_fnreceber.php?id=<?=$fnreceber->id?>&amp;TP=2">Exclui</a></button>
						  </td>
                          <!--<td></td>-->
                        </tr>
                        <?php endforeach;?>	
